...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    // Rileva se l'app è in modalità statica (ssr: false)
    const isStatic = nuxt.options.ssr === false
    nuxt.options.runtimeConfig.public.odooIsStatic = isStatic

    // Registra i server handlers solo se NON è modalità statica
    if (!isStatic) {
      addServerHandler({
